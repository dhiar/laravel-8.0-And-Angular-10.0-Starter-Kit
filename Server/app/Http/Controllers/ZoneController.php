...
class ZoneController extends Controller
{
    ...
    public function fetchZones(Request $request) {
        if($request->searchText){
            $Zones = $this->Zone->fetchZonesForSearch($request->searchColumn,$request->searchText);
            return response()->json($Zones);
        }
        if($request->page && !$request->data_sort_order){
            $Zones = $this->Zone->fetchZonesByPage();
            return response()->json($Zones);
        }
        else if($request->data_sort_order){
            $Zones = $this->Zone->fetchZonesBySorting($request->sorted_colum,$request->data_sort_order);
            return response()->json($Zones);
        }
        $Zones = $this->Zone->fetchZones();
        return response()->json($Zones);

    }

    public function activefetchZones(){
      $Zones = $this->Zone->actvfetchZones();
      return response()->json($Zones);
    }
    ...
    public function addZone(Request $request) {
       ...
       $id = $this->Zone->insertGetId($request);
       $Zones = $this->Zone->fetchZones();

       return response()->json(['zones' => $Zones, 'added_zone_id' => $id]);
    }
    ...
    public function updateZone(Request $request) {
        ...
        if($request->hasFile('Zone_flag')){
            $validator = Validator::make($request->all(), [
                'Zone_flag' => 'required|image|mimes:jpeg,png,jpg,gif,svg|max:2048',
            ]);
            if ($validator->fails())
            {
                $errors_array = array();
            ...
            $image = $request->file('Zone_flag');
            $filename    = time().'_Zone.'.$image->getClientOriginalExtension();
            $destinationPath = public_path('/images/Zone_flag/');
            $image->move($destinationPath, $filename);
            $file_url = '/images/Zone_flag'.'/'. $filename;

        }
        else{
            $file_url = $request->Zone_flag;
        }

        $this->Zone->UpdateRecord($request);
        $Zones = $this->Zone->fetchZonesByPage();
        return response()->json($Zones);
    }
